...
private fun CategoryListItemPreview() {
    LazyRow {
        ...
        item { CategoryItem(text = "전체", onClick = {}, isSelected = false) }
        items(listOf("키워드", "태그")) { CategoryItem(text = it, onClick = {}, isSelected = false) }
...
@Composable
@Preview(showBackground = true, backgroundColor = 0xFF000000)
private fun CategoryFillMaxWidthItemPreview() {
    LazyColumn {
        items(listOf("키워드키워드키워드키워드키워드", "태그")) {
            CategoryFillMaxWidthItem(text = it, onClick = {}, deleteClick = {})
        }
    }
}
